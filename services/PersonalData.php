...
class PersonalData
{
    ...
    public function check(array $personal_data): array
    {
        ...
        if ($person_correct === 'n') {
            ...
            Misc::my_print("d'oh. but no problem, let's set up your personal data again.");

            return $this->set_up_personal_data();
        }

        $contract_correct = Misc::my_read("... and your contract type is still {$personal_data['contract']['text']}? (Y/n) ");

        if ($contract_correct === 'n') {

            Misc::my_print("d'oh. so let's update your contract.");
    ...
    public function set_up_personal_data(): array
    {
        ...
        $last_name = Misc::my_read('whats your last name? ', 0);
    ...
    {
        return Misc::my_switch('whats your contract type? ', [
            [
                'value' => 40,
                'text' => 'full time /40h',
            ],
